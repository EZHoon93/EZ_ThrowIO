...
[ExecuteInEditMode]
public class UltimateJoystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler , ISettingUI
{
    #region EZ
    ...
    public bool IsDeadZone = false; //내가만듬.. 대드존들어갔을때
    ...
    public void OnDrag(PointerEventData touchInfo)
    {
        ...
        if (isSetting)
        {
            ...
            var x = ((100 * touchInfo.position.x) - (50 * textureSize)) / (canvasRectTrans.sizeDelta.x - textureSize);
            var y = ((100 * touchInfo.position.y) - (50 * textureSize)) / (canvasRectTrans.sizeDelta.y - textureSize);
    ...
    {
        positionHorizontal = joystickSetting.vector2.x;
        positionVertical = joystickSetting.vector2.y;
        joystickSize = joystickSetting.size;
        UpdateJoystickPositioning();
    }

    public string GetName() => joystickName;
    ...
    {
        return new Vector2(positionHorizontal, positionVertical);
    }
}